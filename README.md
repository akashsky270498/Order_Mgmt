...
Instead of deploying 4 separate physical servers (which adds massive DevOps overhead for a system of this size), we used a **Modular Monolith** pattern.
...
* **ACID Transactions & Row-Level Locking:** When placing an order, we use `transaction.atomic()` and `select_for_update()`. This locks the specific product row in MySQL to prevent **race conditions** if two users attempt to buy the last item at the exact same millisecond.
...
* **Modular Monolith vs Physical Microservices:** Given the memory constraints (4GB RAM) and the overhead of Docker/Kubernetes, a physical microservice architecture would fail to run locally. The Modular Monolith offers the exact same logical separation of concerns but runs extremely fast and lightweight.
